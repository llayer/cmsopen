...
def hlFeatures(jets, tau, met):
    ...
    input_vectors.append(np.array([tau.px, tau.py, tau.pz]))

    # Jets
    for jet in jets:
        jet_4vec = ROOT.TLorentzVector(jet.px, jet.py, jet.pz, jet.e)
        input_vectors.append(np.array([jet.px, jet.py, jet.pz]))
        H += jet_4vec.E()
        HT += jet_4vec.Et()
        p4 += jet_4vec

    # MET
    met_vec = ROOT.TLorentzVector(met.px, met.py, met.pz, met.e)
    ...
    hl['chargeEta']   = tau.charge * abs(tau.eta)
    hl['met']         = met_vec.Et()
    ...
    hl['mt'] = np.sqrt(((tau_vec.Et() + met_vec.Et())**2) - ((tau.px + met_vec.Px())**2) -
                        ((tau.py + met_vec.Py())**2))
    hl['mTauJet'] = p4.M()
